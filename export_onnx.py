...
class MCnet(nn.Module):
    def __init__(self, block_cfg, detector=True, det_head_branch=None, device="cuda:0"):
        super(MCnet, self).__init__()
        self.det = detector
        self.det_head_branch = det_head_branch
        layers, save = [], []
        self.nc = 1  # traffic or not
        self.detector_index = -1
        self.det_out_idx = block_cfg[0][0]
        self.seg_out_idx = block_cfg[0][1:] if self.det else [n - len(self.det_head_branch) for n in block_cfg[0][1:]]
        self.num_anchors = 3
    ...
    def forward(self, x):
        ...
        if self.det:
            return torch.squeeze(det_out, dim=0), torch.squeeze(torch.max(out[0], 1)[1], dim=0), torch.squeeze(torch.max(out[1], 1)[1], dim=0)   # det, da, ll
        else: 
            return torch.squeeze(torch.max(out[0], 1)[1], dim=0), torch.squeeze(torch.max(out[1], 1)[1], dim=0)  
...
if __name__ == "__main__":
    ...
    parser.add_argument('--do_det', type=bool, default=False, help="whether use det branch") 
    ...
    parser.add_argument('--onnx_path', type=str, default='./weights/yolopv2_384x640_bn.onnx',
                         help="onnx model save path") 
    parser.add_argument('--weight', type=str, 
                        default="./weights/epoch-best-1.0.pth",
                        help="pytorch model path") 

    args = parser.parse_args()

    # yolopv2
    det_head_branch = [i for i in range(27, 38)]
    da_seg_head_branch = [i for i in range(38, 51)]

    device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
    model = MCnet(YOLOPv2, detector=args.do_det, det_head_branch=det_head_branch)
    ...
    if not args.do_det:
        for k, v in checkpoint['state_dict'].items():
            num = int(k.split(".")[1])
            if num in det_head_branch:
                continue
            elif num > det_head_branch[-1]:
                sp = k.split(".")
                sp[1] = str(num-len(det_head_branch))
                k = '.'.join(sp)
            new_dict[k] = v

    model.load_state_dict(checkpoint['state_dict'], strict=False) if args.do_det else model.load_state_dict(new_dict)
    model.fuse()
    model.to(device)
    model.eval()

    yoloponnx = YOLOPONNX(model, use_bn=True, device=device)
    ...
    out_names = ['det', 'da', 'll'] if args.do_det else ['da', 'll']
    torch.onnx.export(yoloponnx, inputs, args.onnx_path,
                      verbose=True, opset_version=12, input_names=['image'], output_names=out_names)
