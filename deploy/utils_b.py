...
def FindPts(ll_mask):
    ...
    ll_open = cv2.morphologyEx(ll_mask, cv2.MORPH_ERODE, element)
    element = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
    ll_open = cv2.morphologyEx(ll_mask, cv2.MORPH_ERODE, element)
    ...
    for i in range(len(contours)):  
        ...
        for j in range(int(contours[i].shape[0])):
            ...
            ret, p = findNearPt(ll_open, p1, 10)
            if ret:
                # if p[0] in x_labels:
                #     continue
                # x_labels.append(p[0])
    ...
    for pts in ll_pts:
        approx = cv2.approxPolyDP(pts, 5, False)
        if approx.shape[0] < 2:
            continue
        
        approx = nms_pt(approx, 20)
        for n in range(approx.shape[0]):
            p = approx[n][0].tolist()
            cv2.circle(ll_mask, tuple(p), 3, (0, 0, 255), -1)
            cv2.putText(ll_mask, f"{n}", tuple([p[0]+5, p[1]]), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 255), 1)
    cv2.imwrite("./inference/output/ll_mask_draw.png", ll_mask)
            ...
            if len(approx_curve) == 2:
                ...
                if left_flag:
                    if not left_curve.exist:  # 第一条曲线
                        ...
                        if (intersect > left_curve.intersect) or (approx_list.length[i] > 1.5 * left_curve.length):
                            # left_curve.exist = True
                            left_curve.p1 = p1
                            left_curve.p2 = p2
                            left_curve.p3 = p3
                    ...
                    if not right_curve.exist:  # 第一条曲线
                        ...
                        # 如果车道线更靠近车子
                        if (intersect < right_curve.intersect) or (approx_list.length[i] > 1.5 * right_curve.length):  
                            # left_curve.exist = True
                            right_curve.p1 = p1
                            right_curve.p2 = p2
                            right_curve.p3 = p3
