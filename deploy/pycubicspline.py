...
def test_spline2d(input_x, input_y):
    ...
    plt.ylabel("y[m]")
    plt.legend()
    plt.savefig("./images/spline-2d-1.png")
    ...
    plt.savefig("./images/spline-2d-2.png")
    ...
    plt.savefig("./images/spline-2d-3.png")
    # plt.show()
...
def test_spline():
    ...
    plt.grid(True)
    plt.axis("equal")
    plt.savefig("./images/spline-1d.png")
    # plt.show()
...
if __name__ == '__main__':
    ...
    imgPath = "./images/ll_output_net.png"
    ...
    ll_pts = FindPts(img_binary)
    for i in range(len(ll_pts)):
        X, Y = [], []
        for p in ll_pts[i]:
            X.append(p[0])
            Y.append(p[1])
        test_spline2d(input_x=X[::10], input_y=Y[::10])
