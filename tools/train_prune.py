...
def main():
    ...
    for epoch in range(begin_epoch+1, cfg.TRAIN.END_EPOCH+1):
        ...
        if val_flag:
            ...
            fi_ll = fitness(np.array(ll_segment_results).reshape(1, -1), item="lane") # 车道线检测评价指标
            if cfg.TRAIN.SEG_ONLY or cfg.TRAIN.ENC_SEG_ONLY:
                fi = fi_da + fi_ll
            elif cfg.TRAIN.DET_ONLY or cfg.TRAIN.ENC_DET_ONLY:
                fi = fi_det
            elif cfg.TRAIN.DRIVABLE_ONLY:
                fi = fi_da
            elif cfg.TRAIN.LANE_ONLY:
