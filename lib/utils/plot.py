...
def show_seg_result(img, result, index, draw_da=True, save_dir=None,palette=None,is_demo=False,is_gt=False):
    # img = mmcv.imread(img)
    # img = img.copy()
    # seg = result[0]
    if palette is None:
    ...
    if not is_demo:
        ...
        if draw_da:
            color_area[result == 1] = [0, 255, 0]   # da
