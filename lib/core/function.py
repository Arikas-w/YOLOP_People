...
def train(cfg, train_loader, model, criterion, optimizer, scaler, epoch, num_batch, num_warmup,
    ...
    start = time.time()
    if rank in [-1, 0]:
        print(('\n' + '%10s' * 2 + '%6s' * 3 + '%10s') % ('Epoch', 'gpu_mem', 'det', 'da', ' total', 'lr',))
    ...
    for i, (input, target, paths, shapes) in pbar:
        ...
        if rank in [-1, 0]:
            ...
            pbar.set_description(('%10s' * 2 + '%6.3f' * 3 + '%10.6f') % (f'{epoch}/{cfg.TRAIN.END_EPOCH - 1}', mem, np.sum(head_losses[:3]), \
                head_losses[3], head_losses[4], optimizer.param_groups[0]['lr']))
...
def validate(config, val_loader, model, criterion, output_dir, device='cpu', half=True, rank=-1):
    """
    validata
    ...
    for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader)):
        if not config.DEBUG:
            ...
            for tgt in target:
                assign_target.append(tgt.to(device))
            target = assign_target
            # nb, _, height, width = img.shape    #batch size, channel, height
        ...
        with torch.no_grad():
            ...
            #Compute loss
            total_loss, head_losses = criterion((train_out,da_seg_out), target, shapes,model)   
            ...
            da_predict_t = []
            da_gt_t = []
            for i, _ in enumerate(da_seg_out):
                pad_w, pad_h = shapes[i][1][1]
                pad_w = int(pad_w)
                pad_h = int(pad_h)
                _, height, width = img[i].shape   # padding 之后的宽和高
                
                # 去除padding
                da_predict_t.append(da_predict[i, pad_h:height-pad_h, pad_w:width-pad_w])
                da_gt_t.append(da_gt[i, pad_h:height-pad_h, pad_w:width-pad_w])
            da_predict = torch.tensor(np.array([t.cpu().detach().numpy() for t in da_predict_t])).to(device)
            da_gt = torch.tensor(np.array([t.cpu().detach().numpy() for t in da_gt_t])).to(device)
            ...
            for i in range(len(img)):
                 # det to pixels
                target[0][(target[0][:, 0] == i), 2:] *= torch.Tensor([width, height, width, height]).to(device) 
                    
                if 0:
                    image = cv2.imread(paths[i])
                    image, ratio, (dw, dh) = letterbox_for_img(image)
                    bboxes = target[0][(target[0][:, 0] == i), :].data.cpu().numpy()
                    for bbox in bboxes:
                        x1 = int(bbox[2] - bbox[4] / 2)
                        y1 = int(bbox[3] - bbox[5] / 2)
                        x2 = int(bbox[2] + bbox[4] / 2)
                        y2 = int(bbox[3] + bbox[5] / 2)
                        cv2.rectangle(image, (x1, y1), (x2, y2), (0, 0, 255), 1)
                    cv2.imwrite("image.jpg", image) 
            lb = [target[0][target[0][:, 0] == i, 1:] for i in range(len(img))] if save_hybrid else []  # for autolabelling
            ...
            if config.TEST.PLOTS:
                if batch_i == 0:
                    for i in range(test_batch_size):
                        ...
                        _ = show_seg_result(img_test, da_seg_mask, i, save_dir)
                        _ = show_seg_result(img_test1, da_gt_mask, i, save_dir, is_gt=True)
